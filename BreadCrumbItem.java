 ...
 * <dd>Selection</dd>
 * </dl>
 * URL https://github.com/Haixing-Hu/swt-widgets/tree/master/src/main/java/com/github/haixing_hu/swt/toolbar
 */
public class BreadCrumbItem extends Item {

  private static final int MIN_WIDTH = 40;
  private static final int MARGIN = 4;
  private static Color COLOR_ETAT_DONE = new Color(Display.getCurrent(), 231, 241, 255);
  private static Color COLOR_ETAT_SELECTED = new Color(Display.getCurrent(), 183, 211, 250);
  private static Color COLOR_ETAT_TODO = new Color(Display.getCurrent(), 250, 230, 231);
  private static Color COLOR_ETAT_DISABLED = new Color(Display.getCurrent(), 203, 203, 203);
  
  // Enum pour l'état du BreadCrumItem (Désactivé, fait, sélectionné, à faire)
  public enum EtatBreadCrumItemEnum { DISABLED, DONE, SELECTED, TODO };
  ...
  private EtatBreadCrumItemEnum etat;
  ...
  public BreadCrumbItem(final BreadCrumb parent, final int style) {
    ...
    this.enabled = true;
    this.etat = EtatBreadCrumItemEnum.TODO;
  ...
  private static int checkBits(int style, final int int0, final int int1,
      final int int2) {
  ...
  private Point computeDefaultSize() {
    ...
    return new Point((2 * MARGIN) + sizeOfTextAndImages.x, (2 * MARGIN)
        + sizeOfTextAndImages.y);
  }
   ...
   * are relative to the receiver's parent (or its display if its parent is
   * null).
   * <p>
  ...
  public void setSelection(final boolean selected) {
    checkWidget();
    if (selected) {
    	this.etat = EtatBreadCrumItemEnum.SELECTED;
    }
   ...
   *
   * @param textColor
   *          the text color to display on the receiver (may be null)
   *
   * @exception IllegalArgumentException
   *              <ul>
   *              <li>ERROR_INVALID_ARGUMENT - if the image has been disposed</li>
   ...
   * setting the tool tip text to an empty string replaces the default, causing
   * no tool tip text to be shown.
   * <p>
  ...
  public void setEtat(EtatBreadCrumItemEnum etat) {
	  checkWidget();
	  this.etat = etat;
	  if (this.etat.equals(EtatBreadCrumItemEnum.SELECTED)) {
		  this.selection = true;
	  } else {
  ...
  public EtatBreadCrumItemEnum getEtat() {
	  checkWidget();
	  return this.etat;
  }
  ...
  /*  if (this.selection) {
    ...
    if (this.etat == EtatBreadCrumItemEnum.DONE) {
    	drawBackgroundAtPosition(x, COLOR_ETAT_DONE);
	} else if (this.etat == EtatBreadCrumItemEnum.SELECTED) {
		drawBackgroundAtPosition(x, COLOR_ETAT_SELECTED);
	} else if (this.etat == EtatBreadCrumItemEnum.TODO) {
		drawBackgroundAtPosition(x, COLOR_ETAT_TODO);
	} else if (this.etat == EtatBreadCrumItemEnum.DISABLED) {
		drawBackgroundAtPosition(x, COLOR_ETAT_DISABLED);
	}
    ...
    this.gc.setForeground(COLOR_ETAT_TODO);
    this.gc.setBackground(COLOR_ETAT_TODO);
  }

  private void drawBackgroundAtPosition(final int x, Color color) {
	    ...
	    final int rectWidth = getWidth() - borderWidth - leftSide 
	        - (this.isLastItemOfTheBreadCrumb && hasBorder ? 1 : 0);
  ...
  private void drawTrianglesAtPosition(final int x) {
    ...
    if (this.parentBreadcrumb.hasBorder) {
      ...
      this.gc.drawLine(x + getWidth(), this.toolbarHeight - 1, x + getWidth()
          + 3, this.toolbarHeight - 1);
    }
  }
  ...
  private Point computeSizeOfTextAndImages() {
    ...
    final boolean textISNotEmpty = (getText() != null)
        && ! getText().equals("");
  ...
  private void drawTextAtPosition(final int xPosition) {
    ...
    int padding;
    if ((this.parentBreadcrumb.indexOf(this) == 0)
        || this.isLastItemOfTheBreadCrumb) {
      padding = 5;
    } else {
      padding = 5;
    }
  ...
  BreadCrumbItem setIsLastItemOfTheBreadCrumb(
      final boolean isLastItemOfTheBreadCrumb) {
